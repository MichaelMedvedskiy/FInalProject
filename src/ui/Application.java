...
public class Application extends JFrame {
    ...
    public Application() {
        ...
        map().addMouseMotionListener(new MouseAdapter() {
            ...
            public void mouseMoved(MouseEvent e) {
               ...
               // System.out.println("Mouse event occured, current position is: "+ pos);
                ...
                String text="124 BANGO";
                BufferedImage img= Util.defaultImage;
                HTMLDocument htmlDocument = null;
                for(MapMarker m :  markers){
                    MapMarkerComplex ma = (MapMarkerComplex) m;
                   // System.out.println("There are Tweets: " + ma.getText());
                  //  htmlDocument.insertAfterStart(); "\"<img src = \\\"\"+ma.getUrl() + \"\\\">\""
                    text = getHtml(ma.getUrl(),ma.getText());
                            //+ ma.getText();

                   // img = ma.getImg();
                }
                //map().paint(img.createGraphics());
    ...
    private Set<Layer> getVisibleLayers() {
        ...
        for (Query q : queries) {
            if (q.getVisible()) {
                ans.add(q.getLayer());
               // System.out.println("the query is: " + q.getLayer());
            }
    ...
    private List<MapMarker> getMarkersCovering(ICoordinate pos, double pixelWidth) {
        ...
        for (MapMarker m : map().getMapMarkerList()) {
           // System.out.println("Here is the layer of the marker: " + m.getLayer());
       ...
       // System.out.println("there are " + ans.size());
        return ans;
    }
